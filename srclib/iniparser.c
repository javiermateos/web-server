/*****************************************************************************
 * ARCHIVO: iniparser.c
 * DESCRIPCION: Libreria para cargar la configuracion de inicialización
 * del servidor web.
 *
 * AUTORES: nowl
...
enum States { START, NEW_SECTION, IN_SECTION, END_OF_FILE };
...
static void trim(char* str, int* begin_ind, int* end_ind)
{
    int b = *begin_ind;
    int e = *end_ind;
    int i;
...
static int read_line(struct read_ini* read_ini)
{
    ...
    for (;;) {
        ...
        if (i >= read_ini->tmp_cap) {
            read_ini->tmp_cap *= 2;
            read_ini->tmp = realloc(read_ini->tmp,
                                    sizeof(*read_ini->tmp) * read_ini->tmp_cap);
        }
    ...
    for (;;) {
        ...
        if (new) {
            ...
            if (section->configs) {
                section->num_configs++;
                section->configs =
                  realloc(section->configs,
                          sizeof(*section->configs) * section->num_configs);
            } else {
        ...
        switch (read_ini->state) {
            ...
            case NEW_SECTION: {
                ...
                if (ini->sections) {
                    ini->num_sections++;
                    ini->sections =
                      realloc(ini->sections,
                              sizeof(*ini->sections) * ini->num_sections);
                } else {
